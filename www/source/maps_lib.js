...
$.extend(MapsLib, {
  ...
  //COMMENTED BY MEK TO ALLOW FOLLOWING OF USER
  map_centroid:       new google.maps.LatLng(42.666011, -77.953491), // center on WNY if all else fails
  defaultZoom:        9,
  ...
  initialize: function() {
   ...
   var myOptions = {
    ...
    //ABOVE IS NEW FEATURE TO ADD ANOTHER FUSAION TABLE OR IMAGE OVERLAY

    // BELOW IS TO ADD WEATHER AND CLOUD LAYERS
  ...
  $(function() {
    ...
    $('#weather_box').click(function(){
        if ($(this).is(':checked'))
            weatherLayer.setMap(MapsLib.map);
        else
            weatherLayer.setMap(null);
       // alert('Show/Hide Weather?' + $(this).is(':checked'));
    });
    $('#clouds_box').click(function(){
        if ($(this).is(':checked'))
            cloudLayer.setMap(MapsLib.map);
        else
  ...
  // ABOVE IS TO ADD CLOUD AND WEATHER LAYERS


    // Below added by TRS to allow user to anotate on the map
    ...
    drawingManager.setMap(MapsLib.map);

   
    
// ADDED TO FIND AND WATCH USER LOCATION
var GeoMarker;
...
function followme() {
 ...
 GeoMarker = new GeolocationMarker();
        GeoMarker.setCircleOptions({fillColor: '#5abae7'});

        google.maps.event.addListenerOnce(GeoMarker, 'position_changed', function() {
          MapsLib.map.setCenter(this.getPosition());
          MapsLib.map.fitBounds(this.getBounds());
        });
// Added to recenter the user
      ...
      google.maps.event.addDomListener(window, 'load', followme);
      
      if(!navigator.geolocation) {
        alert('Your browser does not support geolocation');
        }
// BELOW I WAS TRYING TO DRAW LINE AS USER MOVES
/*
    var line = new google.maps.Polyline({
 //  path: [new google.maps.LatLng(GeoMarker), new google.maps.LatLng(GeoMarker, followme)],
    path: [new google.maps.LatLng(43.161030, -77.610922), new google.maps.LatLng(42.995896, -77.645556)],
    strokeColor: "#FF0000",
    strokeOpacity: 1.0,
    strokeWeight: 10,
    map: MapsLib.map
...
*/
// ABOVE I WAS TRYING TO DRAW LINE AS USER MOVES

 
// Added by TRS for Pamormio image library
/*
var panoramioLayer = new google.maps.panoramio.PanoramioLayer();
  panoramioLayer.setMap(MapsLib.map);


  var photoPanel = document.getElementById('photo-panel');
  MapsLib.map.controls[google.maps.ControlPosition.RIGHT_TOP].push(photoPanel);
...
*/
// Above Added by TRS for Pamormio image library
    
    // Dynamically LOAD MARKERS FROM CSV
    var customIcons = {
      dealer: {
       icon: 'images/maprepair.png'
      },
      food: {
        icon: 'images/mapfood.png'
      },
      hotel: {
        icon: 'images/maphotel.png'
      },
      gas: {
        icon: 'images/mapgas.png'
      }
    };

 
  var infoWindow = new google.maps.InfoWindow; 
  downloadUrl("http://trsmap.com/get_data.php", function(data) { 
  ...
  for (var i = 0; i < markers.length; i++) {
    ...
    var live_updates = markers[i].getAttribute("live_updates");
    var point = new google.maps.LatLng(
        parseFloat(markers[i].getAttribute("lat")),
        parseFloat(markers[i].getAttribute("lng")));
    var html = "<b><font size='6'>" + Company + "</font></b> <br/><center><img src='http://trsmap.com/admin/easylogin/avatars/" + avatar + "'></center><br/>" + address + "<br/>" + Phone + "<br/><b><font color='red'><blink>Live Update</blink></font></b><br/><b>" + live_updates + "</b>";
    ...
    bindInfoWindow(marker, MapsLib.map, infoWindow, html);
  }
 });
...
// ABOVE Dynamically LOAD MARKERS FROM CSV



    //ABOVE LOAD MARKER FROM CSV
