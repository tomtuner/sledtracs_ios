...
function addruler() {
	...
	var ruler1 = new google.maps.Marker({
		position: MapsLib.map.getCenter() ,
		map: MapsLib.map,
		draggable: true
	});
	...
	var ruler1label = new Label({ map: MapsLib.map });
	var ruler2label = new Label({ map: MapsLib.map });
...
//	else if (d<=1) return Math.round(d*1000)+"meters";
	else if (d<=1) return Math.round(d*1000)+"yards";
	return d;
}